...
public class UpdateView extends JDialog {
    ...
    public UpdateView(JFrame owner, Student oldStudent) {
        super(owner, "学生修改", true);
        setSize(430, 450);
        setLocationRelativeTo(owner);
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);

        this.oldId = oldStudent.getId();
        // 回显原数据
        idField.setText(oldStudent.getId().trim());
        nameField.setText(oldStudent.getName().trim());
        ageField.setText(String.valueOf(oldStudent.getAge()));
        majorField.setText(oldStudent.getMajor());
        classField.setText(oldStudent.getcLass());
        preferSubjectField.setText(oldStudent.getPreferSubject());
